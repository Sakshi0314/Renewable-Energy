...
years = list(range(2023, 2033))
  ...
  # Set the page title and favicon for the Streamlit app
    
# Define CSS styles for background image and content
page_bg_img = f'''
<style>
    .stApp {{
        background-image: url("data:image/svg+xml;base64,{img_list[1]}");
...
# Define a function to embed Power BI report iframe
def embed_power_bi_report_iframe():
    # Specify the URL of the Power BI report to be embedded in an iframe
    ...
    # Embed the Power BI report iframe in Streamlit
    st.markdown(f'<iframe width="100%" height="500" src="{power_bi_report_url}" frameborder="0" allowFullScreen="true"></iframe>', unsafe_allow_html=True)
 
# Define the main function to create the Streamlit app
def main():
  
    # Set the title for the Streamlit app
    st.title("Streamlit Power BI Report Embedding")
    st.write("Below is the embedded Power BI report.")
    # Embed the Power BI report iframe
    embed_power_bi_report_iframe()
